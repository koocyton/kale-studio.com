...
import {
  REACTOR_GUICE_COORDINATE,
  REACTOR_GUICE_GITHUB,
  REACTOR_GUICE_MAVEN,
  getReactorGuiceListing,
} from "@/data/reactorGuiceContent";
...
export default function ReactorGuicePageContent() {
  const { locale } = useLanguage();
  const c = getReactorGuiceListing(locale);
          ...
          <div className="max-w-3xl animate-hero-in">
            ...
            <div className="flex items-center gap-4 mb-6">
              <div className="w-16 h-16 rounded-2xl overflow-hidden ring-2 ring-violet-500/30 shadow-lg shadow-violet-500/20">
                <Image
                  src="/app-icons/reactor-guice.png"
                  alt="reactor-guice"
                  width={64}
                  height={64}
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide text-white bg-gradient-to-br from-violet-400 to-indigo-500">
                  OSS
                </span>
              </div>
            </div>

            <h1 className="text-4xl sm:text-5xl font-bold tracking-tight text-foreground">reactor-guice</h1>
            <p className="mt-2 text-lg font-medium text-violet-300/80">{c.tagline}</p>
